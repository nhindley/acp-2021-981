...
% COPY OF getVar to cope with type 12 data in netcdf.




function data = nph_getVar(ncid,varid,varargin)
...
if (nargin > 2) && ischar(varargin{end})
    ...
    switch(xtype)
        case 12 % STRING? not currently built in to matlab
            % abort variable extraction
            data = [];
            return
        case 11 % NC_UINT64
            funcstr = [funcstr 'Uint64'];

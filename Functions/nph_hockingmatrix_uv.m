...
%%%% AS nph_hockingmatrix, except we assume that w' is zero. Hopefullythis
%%%% might allow us to get a more consistent estimate of u and v.
...
function OUT = nph_hockingmatrix_uv(vradprime,theta,phi)
...
A = [...
    nansum(s4t.*c4p)            nansum(s4t.*c2p.*s2p)           nansum(2*s4t.*c3p.*sp)      ; ...
    nansum(s4t.*c2p.*s2p)       nansum(s4t.*s4p)                nansum(2*s4t.*s3p.*cp)      ; ...
    nansum(2*s4t.*c3p.*sp)      nansum(2*s4t.*s3p.*cp)          nansum(4*s4t.*c2p.*s2p)     ];

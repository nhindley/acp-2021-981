...
%%%% EDIT: added support for taking means of angles too for lat/lon, but in
%%%% DEGREES not RADIANS
...
%
% options:
% OUT = nph_bin2mat(___,'method',['nanmean'|'nansum'|'meanagle'],'weights',w)
%
%%%%% INPUTS: (1-D)
        ...
        switch weighting
            case 0 % just a standard mean
                switch method
                    ...
                    case {'meanangle'}
                        for i = 1:length(z)
                            sinOUT(xbin(i),ybin(i)) = sinOUT(xbin(i),ybin(i)) + sind(z(i));
                            cosOUT(xbin(i),ybin(i)) = cosOUT(xbin(i),ybin(i)) + cosd(z(i));
                            emptycount(xbin(i),ybin(i)) = 1;
                        end
                        OUT = atan2d(nansum(sinOUT),nansum(cosOUT));
                        OUT(emptycount == 0) = NaN;    
            ...
            case 0 % just a standard mean
                switch method
                    ...
                    case {'nanmean','mean'}
                        ...
                        case {'meanangle'}
                        for i = 1:length(z)
                            sinOUT(xbin(i)) = sinOUT(xbin(i)) + sind(z(i));
                            cosOUT(xbin(i)) = cosOUT(xbin(i)) + cosd(z(i));
                            emptycount(xbin(i)) = 1;
                        end
                        OUT = atan2d(nansum(sinOUT),nansum(cosOUT));
                        OUT(emptycount == 0) = NaN;
